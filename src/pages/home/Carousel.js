...
class Carousel extends Component {
	...
	getProducts(categoryId) {
		...
		let query = this.props.id === '1' ? 'newest' : 'top-selling';
										...
										<ul className="section-tab-nav tab-nav">
											{this.state.categories.map(category => (
												<li key={category.id} className={category.id === this.state.currentCategory ? 'active' : ''}>
													<Link to={`/products/categories/${category.id}`} id={category.id} onClick={this.handleClick} data-toggle="tab">
														{category.name}
													</Link>
												</li>
											))}
														...
														{this.state.products.map(product => (
															<div key={product.id} className="product">
																<div className="product-img">
																	<img src={require(`../../assets/images/${JSON.parse(product.photo)[0]}`)} alt={product.photo} />
