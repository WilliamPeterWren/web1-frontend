...
class ShoppingCart extends Component {
	...
	componentDidUpdate() {
		...
		} else if (this.props.user === 'guest' && this.state.userId !== '') {
			this.getGuestShoppingCartList(localStorage.getItem('cartList'))
		}
	}
	...
	getShoppingCartList() {
		...
		axios.get(`${Api}/product/cart-list/`, {
			...
			let localCartList = localStorage.getItem('cartList') ? JSON.parse(localStorage.getItem('cartList')) : []

			this.setState({
				cartList: [...response.data, ...localCartList],
				loading: false
			})
									...
									this.state.cartList.map((item) => (
										<div key={item.id} className="cart-item">
											<div className="media cart-item-box">
												...
												<img height="100" width="100" className="align-self-start mr-3" src={item.stock.product.photo && require(`../../assets/images/${item.stock.product.photo}`) ? require(`../../assets/images/${item.stock.product.photo}`) : require('../../assets/images/product01.png')} alt={item.stock.product.name} />
